...
# create data
def make_data(ph2frac, ph2truefrac, total):
    """
    Make synthetic data
    ...
    for pair in ph2frac:
        for _ in range(ph2numexamples[pair]): 
            data['premises'].append(pair[0])
            data['hypoths'].append(pair[1])
...
class Encoder(nn.Module):
    def __init__(self, embedding, op='cat'):
        super(Encoder, self).__init__()
        
        self.embedding = embedding
        assert op in ['cat', 'sum'], 'bad op ' + op
        self.op = op
        print('encoder op:', self.op)
    ...
    def forward(self, x):
        ...
        if self.op == 'cat':
            ...
            # TODO: clean all this
            #print('before squeezing:'); print(emb)
            emb = torch.squeeze(emb, 0)
            #print('before summing:'); print(emb)
            emb = torch.sum(emb, 0)
            #print('after summing:'); print(emb) 
            emb = torch.reshape(emb, (1, emb.nelement()))
            #print('after reshaping:'); emb 
        return emb
...
def get_args():
    ...
    parser.add_argument("--adv_lambda", type=float, default=2.0, help="adversarial loss weight for hyp only classifier")
    parser.add_argument("--adv_hyp_encoder_lambda", type=float, default=3.0, help="adversarial weight for hypothesis only encoder")
...
def main(args):
    ...
    train = make_data(train_ph2frac, ph2truefrac_unambig, train_total)
    test = make_data(test_ph2frac, ph2truefrac_unambig, test_total)
    ...
    premise_encoder = Encoder(nn.Embedding(num_embeddings=args.dictionary_size, embedding_dim=args.embedding_dim), op=op) #, padding_idx=0))
    hypoth_encoder = Encoder(nn.Embedding(num_embeddings=args.dictionary_size, embedding_dim=args.embedding_dim), op=op) #, padding_idx=0))

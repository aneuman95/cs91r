...
word_to_idx = {'p': 0, 'a': 1, 'b': 2, 'c': 3} # 'p' is padding symbol

def make_data(ph2frac, ph2truefrac, total, a_mean_vec, a_var_vec, b_mean_vec, b_var_vec, bias_mean_vec_a, bias_var_vec_a, bias_mean_vec_b, bias_var_vec_b):
    """
    Make synthetic data
    
    ph2frac: dictionary from (p,h) pair to fraction of examples
    ph2truefrac: dictionary from (p,h) pair to fraction of true examples
    total: total number of examples
    
    return data: dictionary of {'hyptohs': hypoths, 'premises': premises, 'lbls': lbls} 
                 where hypoths, premises, and lbls are lists 
    """
  
    data = {'hypoths':[], 'premises':[], 'lbls':[], 'ph':[]}
    ph2numexamples = dict()
    for pair in ph2frac:
        ph2numexamples[pair] = round(total * ph2frac[pair])

    ph2numtrueexamples = dict()
    for pair in ph2frac:
        for _ in range(ph2numexamples[pair]): 
            if np.random.random() < ph2truefrac[pair]:
                data['lbls'].append(TRUE)
                ph2numtrueexamples[pair] = ph2numtrueexamples.get(pair, 0) + 1
            else:
                data['lbls'].append(FALSE)
            if pair[0] == 'a' and pair[1] =='a':
                data['premises'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['hypoths'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['ph'].append('aa')
            elif pair[0] == 'a' and pair[1] =='b':
                data['premises'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['hypoths'].append(np.random.normal(b_mean_vec, b_var_vec))
                data['ph'].append('ab')
            elif pair[0] == 'a' and pair[1] =='ac':
                data['premises'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['hypoths'].append(np.random.normal(a_mean_vec, a_var_vec) + np.random.normal(bias_mean_vec_a, bias_var_vec_a))
                data['ph'].append('aac')
            elif pair[0] == 'a' and pair[1] =='bc':
                data['premises'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['hypoths'].append(np.random.normal(b_mean_vec, b_var_vec) + np.random.normal(bias_mean_vec_b, bias_var_vec_b))
                data['ph'].append('abc')
            elif pair[0] == 'b' and pair[1] =='a':
                data['premises'].append(np.random.normal(b_mean_vec, b_var_vec))
                data['hypoths'].append(np.random.normal(a_mean_vec, a_var_vec))
                data['ph'].append('ba')
    ...
    print('num premises:', len(data['premises']))
    print('num hypoths:', len(data['hypoths']))
    print('num lbls:', len(data['lbls']))
    print('ph2numexamples:', ph2numexamples)
    print('ph2numtrueexamples:', ph2numtrueexamples)
...
class HypothNet(nn.Module):
    def __init__(self, hidden_dim, num_classes, adv_hyp_encoder_lambda, encoder, hypoth_length=1, op='cat'):
...
class NLINet(nn.Module):
    def __init__(self, hidden_dim, num_classes, adv_hyp_encoder_lambda, encoder_p, encoder_h, hypoth_length=1, op='cat'):
...
def trainepoch(epoch, train, optimizer_nli, optimizer_hypoth, params, word_vec, nli_net, hypoth_net, \
    ...
    for stidx in range(0, len(hypoths)):
        ...
        if np.random.random() > params.random_premise_frac:
            ...
            p = np.array(premises)
            p = make_tensor(p[np.random.choice(p.shape[0], 1, replace=True)])
...
def evaluate(epoch, valid, optimizer_nli, optimizer_hypoth, params, word_vec, nli_net, hypoth_net, eval_type='valid', final_eval=False, adv_lambda=1.):
    ...
    if adv_lambda > 0:
        return (list(zip(valid['ph'], targets, preds_nli, preds_hypoth))), eval_acc_nli, eval_acc_hypoth
    else:
        return (list(zip(valid['ph'], targets, preds_nli))), eval_acc_nli, eval_acc_hypoth
...
def get_args():
    ...
    parser.add_argument("--adv_lambda", type=float, default=2., help="adversarial loss weight for hyp only classifier")
    parser.add_argument("--adv_hyp_encoder_lambda", type=float, default=2., help="adversarial weight for hypothesis only encoder")
    parser.add_argument("--nli_net_adv_hyp_encoder_lambda", type=float, default=3., help="adversarial weight for hypothesis encoder in NLI net")
    parser.add_argument("--random_premise_frac", type=float, default=0., help="fraction of random premises to use in NLI net")
    parser.add_argument("--input_dim", type=int, default=2, help="dimension of the mutlivariate normal distribution")
    parser.add_argument("--hidden_dim", type=int, default=10, help="dimension of the encoder's hidden layer")

    # model
    parser.add_argument("--num_classes", type=int, default=2, help="number of classes")
    parser.add_argument("--dictionary_size", type=int, default=2, help="dictionary size")
...
def main(args):
    ...
    #a_mean_vec = np.full(args.input_dim, 0.)
    #a_var_vec = np.full(args.input_dim, .5)
    #b_mean_vec = np.full(args.input_dim, 2.)
    #b_var_vec = np.full(args.input_dim, 0.5)
    #bias_mean_vec = np.full(args.input_dim, 20.)
    #bias_var_vec = np.full(args.input_dim, 0.)
    a_mean_vec = np.array([0,0])
    a_var_vec = np.array([1,0])
    b_mean_vec = np.array([5,0])
    b_var_vec = np.array([1,0])
    bias_mean_vec_a = np.array([0,20])
    bias_var_vec_a = np.array([0,1])
    bias_mean_vec_b = np.array([0,20])
    bias_var_vec_b = np.array([0,1])
    ...
    train = make_data(train_ph2frac, ph2truefrac_unambig, train_total, a_mean_vec, a_var_vec, b_mean_vec, b_var_vec, bias_mean_vec_a, bias_var_vec_a, bias_mean_vec_b, bias_var_vec_b)
    test = make_data(test_ph2frac, ph2truefrac_unambig, test_total, a_mean_vec, a_var_vec, b_mean_vec, b_var_vec, bias_mean_vec_a, bias_var_vec_a, bias_mean_vec_b, bias_var_vec_b)
    ...
    premise_encoder = Encoder(args.input_dim, args.hidden_dim) 
    hypoth_encoder = Encoder(args.input_dim, args.hidden_dim)

    nli_net = NLINet(args.hidden_dim, args.num_classes, args.nli_net_adv_hyp_encoder_lambda, premise_encoder, hypoth_encoder, hypoth_length=hypoth_length, op=op)
    #nli_net = NLINet(args.embedding_dim, args.num_classes, args.nli_net_adv_hyp_encoder_lambda, hypoth_encoder, hypoth_encoder)
    hypoth_net = HypothNet(args.hidden_dim, args.num_classes, args.adv_hyp_encoder_lambda, hypoth_encoder, hypoth_length=hypoth_length, op=op)
    print(nli_net)
    print(hypoth_net)
    ...
    while not stop_training and epoch <= args.n_epochs:
        ...
        results_train, eval_acc_nli_train, eval_acc_hypoth_train = evaluate(epoch, train, optimizer_nli, optimizer_hypoth, args, word_vecs, nli_net, hypoth_net, 'train', adv_lambda=args.adv_lambda)
        results_test, eval_acc_nli, eval_acc_hypoth = evaluate(epoch, test, optimizer_nli, optimizer_hypoth, args, word_vecs, nli_net, hypoth_net, 'valid', adv_lambda=args.adv_lambda)
        epoch += 1

    return results_train, results_test, eval_acc_nli_train, eval_acc_hypoth_train, eval_acc_nli, eval_acc_hypoth

if __name__ == '__main__':
    args = get_args()
    if args.gpu_id > -1:
        torch.cuda.manual_seed(seed)
    results_train, results_test, eval_acc_nli_train, eval_acc_hypoth_train, eval_acc_nli, eval_acc_hypoth = main(args)

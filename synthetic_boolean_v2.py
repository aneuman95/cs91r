...
word_to_idx = {'x0': 0, 'x1': 1, 'x2': 2, 'x3': 3, 'x4': 4, 'x5': 5, 'x6': 6, 'x7': 7, 'x8': 8, 'x9': 9,
'x10':10, 'x11': 11, 'x12': 12, 'x13': 13, 'x14': 14, 'x15': 15, 'x16': 16, 'x17': 17, 'x18': 18, 'x19': 19, 'x20': 20,
'And': 21, 'Or': 22, 'Not': 23, "v": 24, '^': 25, '¬': 26, '(': 27, ')': 28} 

class Formula:
    def __init__(self, num_variables, num_clauses, clause_length):
    ...
    def make_formula(self):
        ...
        var_labels = [i for i in range(self.num_variables)]
        ...
        for i in range(self.num_clauses):
            #randomly sample from literals 
            literals = [np.random.choice(variables) for x in range(self.clause_length)]
            z3_literals = [Bool(x) for x in literals]

            #randomly negate each literal 
            for j in range(len(literals)):
                if np.random.uniform() < 0.5:
                    z3_literals[j] = Not(z3_literals[j])
                    literals[j] = '¬' + literals[j]

            #construct Z3, prefix, and infix clauses 
            z3_clauses.append(Or(z3_literals))
            infix_clauses.append('(')
            prefix_clauses.append('Or')
            prefix_clauses.append('(')
            for k in range(len(literals)):
                if literals[k][0] == '¬':
                    infix_clauses.append('¬')
                    infix_clauses.append(literals[k][1:])
                    prefix_clauses.append('Not')
                    prefix_clauses.append('(')
                    prefix_clauses.append(literals[k][1:])
                    prefix_clauses.append(')')
                    if k < len(literals) - 1:
                        infix_clauses.append( "v")
                    else:
                        infix_clauses.append(')')
                        prefix_clauses.append(')')
                else:
                    infix_clauses.append(literals[k])
                    prefix_clauses.append(literals[k])
                    if k < len(literals) - 1:
                        infix_clauses.append( "v")
                    else:
                        infix_clauses.append(')')
        ...
        self.z3 = And(z3_clauses)
        self.infix = infix_clauses
        self.prefix = prefix_clauses
...
#create data 
def make_data(num_examples,num_variables,num_clauses,clause_length):
    #store data in dictionary 
    data = {'hypoths':[], 'premises':[], 'lbls':[]}

    for i in range(num_examples):
        #make premise
        p = Formula(num_variables,num_clauses,clause_length)
        p.make_formula()
        data['premises'].append(p)

        #make hypothesis
        q = Formula(num_variables,num_clauses,clause_length)
        q.make_formula()
        data['hypoths'].append(q)

        #check entailment 
        label = check(p.z3,q.z3)
        if label == "valid":

...
#parse data and store in JSON files  
data = ['train.txt', 'validate.txt', 'test_big.txt', 'test_exam.txt', 'test_hard.txt', 'test_massive.txt', 'test_easy.txt']

lang = propositional_language()
parse_obj = Parser(lang)
...
def make_data(txt_file):
    ...
    for line in lines:
        ...
        hypoth = l[1]
        lbl = l[2]
        parsed_prem = parse_obj.parse(premise)
        parsed_hypoth = parse_obj.parse(hypoth)
        data['premises'].append(parsed_prem[0])
        data['hypoths'].append(parsed_hypoth[0])
        data['lbls'].append(lbl)

    #store data in JSON file 
    with open(txt_file + '_data','w+') as f:
        json.dump(data,f)

    return
    
#data_test_exam = make_data('test_exam.txt')
#data_validate = make_data('validate.txt')
data_validate = make_data('train.txt')
